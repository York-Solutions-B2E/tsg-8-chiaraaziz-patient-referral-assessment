...
function findMatchedOption(authenticators, options) {
  ...
  for (let authenticator of authenticators) {
    option = options.find(({ relatesTo }) => relatesTo.key === authenticator.key);
    if (option) {
      break;
    }
  }
...
function getDataFromIntrospect(authClient, data) {
  return __async(this, null, function* () {
    ...
    if (stateHandle) {
      idxResponse = yield introspect(authClient, { withCredentials, version, stateHandle });
    } else {
...
function fetchRequest(method, url, args) {
  ...
  var fetch = window.fetch || import_cross_fetch20.default;
